...
class SSIM_loss(torch.nn.Module):
    def __init__(self, if_MS=True, win_size=11, data_range=255, channel=1):
...
class MIND_loss_rgb(torch.nn.Module):
    """A lightweight RGB-friendly wrapper for MIND_loss.

    The original MIND implementation below is 3D (conv3d) and expects a single channel.
    For histopathology RGB patches, a common low-risk baseline is to compute MIND on a
    grayscale projection (keeps edges/structures, removes stain-scale differences to some extent).

    Input shapes:
      - y_pred: [B, 3, H, W] or [B, 1, H, W]
      - y_true: [B, 3, H, W] or [B, 1, H, W]
    """

    def __init__(self, win=None, rgb_to_gray: str = 'luma'):
        super().__init__()
        self.base = MIND_loss(win=win)
    ...
    def _to_gray(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # luma weights (sRGB): keeps structure while being deterministic and differentiable
        if self.rgb_to_gray == 'luma':
            w = x.new_tensor([0.2989, 0.5870, 0.1140]).view(1, 3, 1, 1)
            return (x * w).sum(dim=1, keepdim=True)
        elif self.rgb_to_gray == 'mean':
            return x.mean(dim=1, keepdim=True)
        else:
            raise ValueError(f"Unknown rgb_to_gray='{self.rgb_to_gray}'")

    def forward(self, y_pred: torch.Tensor, y_true: torch.Tensor) -> torch.Tensor:
        y_pred_g = self._to_gray(y_pred)
        y_true_g = self._to_gray(y_true)

        # MIND_loss is implemented in 3D; lift 2D -> pseudo-3D with depth=1.
        y_pred_3d = y_pred_g.unsqueeze(2)  # [B,1,1,H,W]
        y_true_3d = y_true_g.unsqueeze(2)
        return self.base(y_pred_3d, y_true_3d)
...
class MutualInformation(torch.nn.Module):
    ...
    def mi(self, y_true, y_pred):
        ...
        vbc = torch.reshape(self.vol_bin_centers, o).to(device=y_pred.device, dtype=y_pred.dtype)
