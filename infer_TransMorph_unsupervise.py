...
def main():
    ...
    batch_size = 1
    # Check these paths - matching the unsupervise training script defaults
    test_dir = '/data2/xujr/idr_data/Train_CrossModal/Test' 
    model_dir = "/data2/xujr/output_model/0330/TransMorph_unsupervised_ncc_1.0_1.0"
    ...
    with torch.no_grad():
        for i, data in enumerate(test_loader):
            ...
            # data structure from MultiModalityPairedDataset
            x = data[0] # Moving Image
            y = data[1] # Fixed Image (Target)
            
            # Since this is an unsupervised cross-modal loader, gt_flow might be missing.
            # We still want to log EPE if it *is* present (for evaluation), otherwise skip EPE.
            has_gt_flow = len(data) >= 3
            if has_gt_flow:
            ...
            # output[0] is deformed image, output[1] is predicted flow
            pred_img = output[0]
            pred_flow = output[1]
            ...
            if i < 5: 
                # Replace gt_flow with pred_flow dummy if it doesn't exist
                gt_visual = gt_flow if has_gt_flow else pred_flow
                save_visual_result(x, y, pred_img, pred_flow, gt_visual, i, output_dir)
                print(f"Saved visual result for sample {i} to {output_dir}")
                print(f"Sample {i} - SSIM: {val_ssim.item():.4f}, ZNCC: {val_ncc.item():.4f}, EPE: {mean_epe.item() if has_gt_flow else 0:.4f}, Folding Ratio: {folding_ratio:.4%}")
    ...
    if eval_epe.count > 0:
        ...
        print('Mean EPE:  N/A (No GT flow provided)')
