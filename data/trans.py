...
class RandomFlip(Base):
    ...
    def tf(self,img,k=0): # img shape is (C, H, W, ...)
        # k=2: if flow passed as 3rd element in datasets.py
        # Assume flow channel layout is (Vertical/Y, Horizontal/X) corresponding to axes (1, 2)
        ...
        if len(self.axis) == 1:
            if self.x_buffer and 1 in self.axis:
                 should_flip_x = True # Flip Axis 1 (Height/Y)
            if self.y_buffer and 2 in self.axis:
                 should_flip_y = True # Flip Axis 2 (Width/X)
        else:
            if self.x_buffer:
                should_flip_x = True # Flip Axis 1 (Height/Y)
            if self.y_buffer:
        ...
        if should_flip_x:
            img = np.flip(img,axis=1) # Flip Height
            if k == 2: # If is flow, negate Y-component (Index 0)
                img[0, ...] = -img[0, ...]
                
        if should_flip_y:
            img = np.flip(img,axis=2) # Flip Width
            if k == 2: # If is flow, negate X-component (Index 1)
                img[1, ...] = -img[1, ...]
