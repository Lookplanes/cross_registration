...
class Base(object):
    ...
    def __call__(self, img, dim=2, reuse=False):
        # image: nhwtc
        # shape: no first dim
        if not reuse:
...
class RandomRotion(Base):
    ...
    def tf(self, img, k=0):
        """The rotation function supports shape [H,W,D,C] or [H,W,D]."""
        bsize = img.shape[0]
...
class Pad3DIfNeeded(Base):
    def __init__(self, shape, value=0, mask_value=0):
        self.shape = shape
        self.value = value
...
class GaussianBlur(Base):
    ...
    def tf(self, img, k=0):
        ...
        for n in range(img.shape[0]):
            ...
            # sample each channel separately to avoid correlations
            if sig > self.eps:
