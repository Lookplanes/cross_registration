"""
Deprecated Cross-Attention Registration Head for historical experiments.

This module is retained only to reproduce pre-conditioned-TransMorph Stage 2
checkpoints.  New registration training must use
...
::
    ...
    │    Q = F₁ · Wq    K,V = F₂' · Wk, Wv                        │
    │    attn = softmax(Q·Kᵀ/√d)                                  │
    │    fused = attn · V  [optionally + F₁ for legacy runs]     │
    └──────────────────────────┬───────────────────────────────────┘
                               │
...
class CrossAttentionFusion(nn.Module):
    ...
    By default, the output is only ``Attention(Q, K, V)`` so the decoder has
    no direct fixed/query-only route.  ``residual="fixed_query"`` reproduces
    the legacy ``Attention(Q, K, V) + F₁`` behaviour.
    """

    def __init__(self, dim: int, num_heads: int = 4, max_tokens: int = 256,
                 residual: str = "none"):
        super().__init__()
        if dim % num_heads != 0:
            raise ValueError(f"dim={dim} must be divisible by num_heads={num_heads}")
        if residual not in {"none", "fixed_query"}:
            raise ValueError(
                "residual must be 'none' or 'fixed_query', "
                f"got {residual!r}"
            )
    ...
    def forward(self, f1: torch.Tensor, f2: torch.Tensor) -> torch.Tensor:
        ...
        # Global attention is quadratic.  Pool high-resolution stages to a
        # bounded token count, then restore the fused feature resolution.
        original_size = (H, W)
        if H * W > self.max_tokens:
            scale = (self.max_tokens / (H * W)) ** 0.5
            pooled_h = max(1, int(round(H * scale)))
            pooled_w = max(1, int(round(W * scale)))
            f1 = F.adaptive_avg_pool2d(f1, (pooled_h, pooled_w))
            f2 = F.adaptive_avg_pool2d(f2, (pooled_h, pooled_w))
...
class MultiScaleCrossAttentionFuser(nn.Module):
    ...
    def __init__(self, feature_dims: list[int], num_heads: int = 4,
        ...
        self.fusers = nn.ModuleList([
            CrossAttentionFusion(d, num_heads, residual=residual)
...
class CrossAttentionRegHead(nn.Module):
    ...
    fusion_residual : str      ``none`` prevents a fixed-only shortcut;
                               ``fixed_query`` reproduces legacy checkpoints.
    """

    def __init__(self, embed_dim: int = 96, out_indices: tuple = (0, 1, 2, 3),
                 reg_head_chan: int = 16, num_heads: int = 4,
                 fusion_residual: str = "none"):
        super().__init__()
        if fusion_residual not in {"none", "fixed_query"}:
            raise ValueError(
                "fusion_residual must be 'none' or 'fixed_query', "
                f"got {fusion_residual!r}"
            )
        ...
        self.fuser = MultiScaleCrossAttentionFuser(
            dims, num_heads, residual=fusion_residual,
        )
