...
class MaskedFlowL1(nn.Module):
    def __init__(self):
        super(MaskedFlowL1, self).__init__()

    def forward(self, pred_flow, gt_flow, valid_mask):

        abs_error = torch.abs(pred_flow - gt_flow)

        masked_error = abs_error * valid_mask
        
        valid_pixel_count = valid_mask.sum() * 2 
        loss = masked_error.sum() / (valid_pixel_count + 1e-8)
...
def main():
    ...
    '''
    if is_supervised:
        # No random flip for now to avoid flow mismatch if not handled carefully, 
        # but datasets.py seems to handle it. Keeping it strictly aligned first.
        train_composed = None 
        ...
        # Default unsupervised legacy code (not used in this supervised run)
    ...
    # 1. Image Similarity Loss (废除)
    # criterion = losses.SSIM_loss(if_MS=False, data_range=1.0, channel=1)
    
    # 2. Supervised L1 Loss on Flow (DDF) - Masked Flow
    criterion_flow = MaskedFlowL1()
    
    # 3. Regularization Loss (Smoothness)
    # 使用 L2 梯度的平滑约束
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        loss_flow_meter = utils.AverageMeter()
        ...
        for data in train_loader:
            ...
            # Unpack data: [fixed, moving, gt_flow] or similar
            # Check datasets.py: returns moving, fixed, flow
            x = data[0] # Moving
            y = data[1] # Fixed
            ...
            if is_supervised:
                if len(data) < 3:
                     raise ValueError("Data loader did not return GT flow. Ensure dataset has 'gt_flow' folder.")
                gt_flow = data[2] # GT Flow [B, 2, H, W]
                ...
                # 1. L1 Loss (DDF_pred vs DDF_gt) with Mask
                loss_flow = criterion_flow(pred_flow, gt_flow, valid_mask) * weights[0]
                
                # 2. Smoothness Regularization on Velocity Field (SVF)
                loss_reg = criterion_reg(pos_flow, None) * weights[1]
                
                loss = loss_flow + loss_reg
                
                loss_flow_meter.update(loss_flow.item(), x.size(0))
                loss_reg_meter.update(loss_reg.item(), x.size(0))
            else:
                # Fallback to unsupervised (Should not happen based on user request)
                pass
            ...
            if idx % print_freq == 0:
                print('Iter {} of {} Loss: {:.6f}, L1(Flow): {:.6f}, Reg: {:.6f}'.format(
                    idx, len(train_loader), loss.item(), loss_flow.item(), loss_reg.item()))
        ...
        if (epoch + 1) % val_interval == 0:
            ...
            with torch.no_grad():
                for data in val_loader:
                    ...
                    # output[2] is SVF (Velocity field)
                    
                    # 1. EPE (End Point Error) with Mask
                    if len(data) >= 4:
                        valid_mask = data[3]
                    else:
                        valid_mask = (y > 1e-4).float()
            ...
            print('Epoch {} Validation: Mean EPE: {:.4f}, Mean SSIM: {:.4f}, Mean ZNCC: {:.4f}'.format(
