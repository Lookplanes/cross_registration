...
def get_2DTransMorphSinRGB_config():
    '''
    2D TransMorph (Sinusoidal Positional Embedding) for RGB inputs.

    Input convention (recommended): concat([fixed_rgb(3ch), moving_rgb(3ch)]) -> 6 channels.
    '''
    config = get_2DTransMorphSin_config()
    config.in_chans = 6
    return config
