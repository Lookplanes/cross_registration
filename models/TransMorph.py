...
class TransMorph(nn.Module):
    def __init__(self, config):
        ...
        # Conv-skip branch takes the raw concatenated input (moving+fixed).
        # Use in_chans from config so the model can support RGB concatenation (e.g. 6 channels).
    ...
    def forward(self, x):
        # x is concatenated input: [fixed, moving]. For grayscale, C=2 and source is the first channel.
        # For RGB concatenation, C=6 and we take the first 3 channels as the source image.
        src_ch = x.shape[1] // 2
        source = x[:, 0:src_ch, :, :]
        if self.if_convskip:
            x_s0 = x.clone()
...
def get_affine_net(config):
    from .TransMorph_affine_2d import TransMorphAffine2D
    return TransMorphAffine2D(config)
